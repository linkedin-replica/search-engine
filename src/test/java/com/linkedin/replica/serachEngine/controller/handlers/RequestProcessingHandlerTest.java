...
public class RequestProcessingHandlerTest {
	...
	public static void setup() throws IOException {
		...
		String[] a = { "src/main/resources/app.config", "src/main/resources/arango.test.config",
	...
	public void testProcessing() {
		...
		assertEquals("Wrong message", "Changes are applied successfully and configuration files are updated",
				response.get("message"));
	}

	@Test
	public void testAddingCommand() throws IOException, ClassNotFoundException, InstantiationException,
	...
	@Test
	public void testDeletingCommand() throws IOException, ClassNotFoundException, InstantiationException,
			IllegalAccessException, NoSuchMethodException, SecurityException, IllegalArgumentException,
			InvocationTargetException {
		// add a new command file by sending file to RequestProcessingChannel
		addCommandFile();
	...
	public void testMissingParameter() throws IOException {
		...
		// add handler class name that will be added to command configuration
		// file
		...
		/*
		 * read result from channel which is null because an exception is thrown
		 * due to empty body and the exception will be caught and an error
		 * response is created and flushed to responseDecoderHandler so no
		 * inbound message to be read
		 */
		...
		assertEquals(
				"Wrong errMessage",
				"Invalid parameters : [fileName, bytes, handler]. expected : [fileName, configPropKey, handler, bytes]",
				err.get("errMessage"));
	}
