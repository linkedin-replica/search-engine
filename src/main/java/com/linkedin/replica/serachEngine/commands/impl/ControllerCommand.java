...
public class ControllerCommand extends Command {
	...
	/**
	 * set the maxium number of threads that could be created by the thread pool
	 * library
	 * 
	 * @param val
	 */
	public static void setMaxThreadCount(Object val) {
		if (val == null || !((JsonPrimitive) val).isNumber())
			throw new SearchException(String.format(
					"Invalid parameters : %s. expected an integer representing maximum number of threads", val));
	...
	/**
	 * set the maximum number of database connections that could be created to
	...
	public static void setMaxDBConnectionsCount(Object val) {
		if (val == null || !((JsonPrimitive) val).isNumber())
			throw new SearchException(String.format(
					"Invalid parameters : %s. expected an integer representing maximum number of DB connections", val));
	...
	public static void addCommand(Object val) throws IOException {
		...
		// validate embedded JSON object obj to check that all obligatory key
	...
	public static void updateCommand(Object val) throws IOException {
		// addCommand will delete file and write the updated one coming in
		// request body
		addCommand(val);
	}

	/**
	 * freeze the independent app i.e. stop accepting new requests and release
	 * all resources from pools
	 * 
	 * @param val
	 * @throws TimeoutException
	 * @throws IOException
	 */
	public static void freeze(Object val) throws IOException, TimeoutException {
		MessageReceiver.getInstance().closeChannel();
	}
	 ...
	 * @throws IOException
	 */
	public static void resume(Object val) throws IOException, TimeoutException {
		MessageReceiver.getInstance().openChannel();
	}
	...
	public static void setErrorReportingLevel(Object val) {
		if (val == null || !((JsonPrimitive) val).isNumber())
			throw new SearchException(String.format(
					"Invalid parameters : %s. expected an integer representing logging level.", val));
	...
	private static void validate(JsonObject obj, String... args) {
		for (String arg : args) {
			if (obj.get(arg) == null)
				throw new SearchException(String.format("Invalid parameters : %s. expected : %s", obj.keySet()
